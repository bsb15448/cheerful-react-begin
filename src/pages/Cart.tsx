...
import { initKonnectPayment } from '@/utils/paymentApi';
...
const Cart = () => {
  ...
  const onSubmit = (data: OrderForm) => {
    // This function is no longer used for payment processing
    // It's kept for potential form validation logic
  };
  ...
  const handlePayment = async () => {
    if (!hasInfoSaved || !isFormValid) {
      toast({
        description: "Veuillez remplir et sauvegarder vos informations",
        variant: "destructive",
        duration: 2000,
      });
      return;
    ...
    try {
      ...
      if (total === 0) {
        navigate(paymentResponse.payUrl);
      } else {
      ...
      toast({
        description: "Le traitement du paiement a échoué. Veuillez réessayer.",
        variant: "destructive",
        duration: 3000,
      });
    }
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-6 rounded-lg shadow-sm space-y-4 sticky top-4">
            ...
            <Button 
              ...
              size="lg"
              disabled={!isFormValid || !hasInfoSaved || paymentProcessing}
            ...
            {hasInfoSaved && isFormValid && !paymentProcessing && (
