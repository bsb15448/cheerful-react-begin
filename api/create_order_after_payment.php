...
try {
    ...
    try {
        ...
        $item_query = "INSERT INTO order_items (id, order_id, child_id, child_name, child_age, child_objective, child_message, child_photo_url, child_eye_color, child_remarks, book_title, book_description, price) 
                      VALUES (:id, :order_id, :child_id, :child_name, :child_age, :child_objective, :child_message, :child_photo_url, :child_eye_color, :child_remarks, :book_title, :book_description, :price)";
        $item_stmt = $db->prepare($item_query);

        // Child insert/update query with proper UUID handling
        $child_upsert_query = "INSERT INTO children (id, user_id, name, age, objective, message, photo_url, eye_color, remarks) 
                              VALUES (:id, :user_id, :name, :age, :objective, :message, :photo_url, :eye_color, :remarks)
        ...
        foreach ($children as $child) {
            try {
                ...
                $child_eye_color = $child['eyeColor'] ?? '';
                $child_remarks = $child['remarks'] ?? '';
                ...
                $child_stmt->bindParam(':eye_color', $child_eye_color);
                $child_stmt->bindParam(':remarks', $child_remarks);
                $child_stmt->execute();
                ...
                $item_stmt->bindParam(':child_eye_color', $child_eye_color);
                $item_stmt->bindParam(':child_remarks', $child_remarks);
