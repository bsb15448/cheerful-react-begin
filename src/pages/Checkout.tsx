...
const Checkout = () => {
  ...
  const isFormValid = formData.email && formData.email.trim() !== '' && formData.name && formData.name.trim() !== '' && formData.phone && formData.phone.trim() !== '' && formData.address && formData.address.trim() !== '' && formData.city && formData.city.trim() !== '' && formData.postalCode && formData.postalCode.trim() !== '';
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (user?.id) {
        ...
        generatedGuestId = sessionStorage.getItem('mylittlehero_guest_id') || crypto.randomUUID();
      ...
      const processedChildren = await Promise.all(children.map(async (child: any, idx: number) => {
        // If already has photoUrl, keep as is
        if (child.photoUrl) return {
          ...child,
        ...
        if (child.photoData && !child.photoUrl) {
          try {
            ...
            if (uploadResp.success && uploadResp.data?.image_url) {
              return {
                ...child,
                photoUrl: uploadResp.data.image_url
              };
            } else {
      ...
      const {
        sessionId,
        url
      } = await paymentService.createStripeCheckout(checkoutData);
  ...
  return <div className="min-h-screen relative overflow-hidden font-baloo" style={{
    background: 'linear-gradient(135deg, #E8D5FF 0%, #F3E8FF 25%, #E0E7FF 50%, #F0F4FF 75%, #F8FAFF 100%)'
      ...
      <div className="container mx-auto px-3 md:px-4 py-4 md:py-8 relative z-10 py-8 md:py-12">
        ...
        <div className="block lg:hidden space-y-4 md:space-y-6">
          ...
          <div className="bg-white/95 backdrop-blur-sm rounded-xl p-3 shadow-lg border border-green-100">
            <div className="flex items-center justify-center mb-3">
              <img src="https://i.ibb.co/9mjzbfTr/image.png" alt="Trustpilot" className="h-6 object-contain" />
            </div>
              ...
              <div className="bg-green-50 p-2 rounded-lg border-l-4 border-green-400">
                <div className="flex items-center mb-1">
                  <div className="flex mr-1">
                    {[...Array(5)].map((_, i) => <Star key={i} className="w-2 h-2 text-green-500 fill-current" />)}
                  </div>
                  <CheckCircle className="w-2 h-2 text-green-500 ml-1" />
                </div>
                <p className="text-slate-700 font-medium">"Mon fils a reconnu son prénom dès la première page." Il m'a regardée avec de grands yeux et m'a dit : "C'est moi?"  Depuis, il veut qu'on relise le livre tous les soirs.</p>
                <span className="text-slate-500 text-xs">⸻ Sophie M.</span>
              </div>
              <div className="bg-green-50 p-2 rounded-lg border-l-4 border-green-400">
                <div className="flex items-center mb-1">
                  <div className="flex mr-1">
                ...
                <p className="text-slate-700 font-medium">"Elle était super fière de se voir en héroïne." C'est la première fois qu'elle finit un livre d'un coup. Elle me l'a même raconté à sa manière à la crèche.</p>
                <span className="text-slate-500 text-xs">⸻ Marie-Claire L.</span>
              </div>
              <div className="bg-green-50 p-2 rounded-lg border-l-4 border-green-400">
                <div className="flex items-center mb-1">
                  <div className="flex mr-1">
                ...
                <p className="text-slate-700 font-medium">"Il a eu un déclic." On a choisi le thème 'confiance'. Et franchement, je vois la différence. Il prend plus la parole et se tient plus droit.</p>
                <span className="text-slate-500 text-xs">⸻ Camille R.</span>
              </div>
            </div>
          ...
          <div className="bg-white/95 backdrop-blur-sm rounded-xl p-4 shadow-lg">
            <div className="flex items-center mb-4">
              <div className="w-6 h-6 bg-slate-700 rounded-full flex items-center justify-center shadow-md mr-3">
                <span className="text-white text-sm font-bold">1</span>
              </div>
              <h3 className="text-lg font-bold text-slate-700">Résumé de commande</h3>
            </div>
            
            <div className="space-y-3 mb-4">
              {children.map((child: any, index: number) => <div key={index} className="flex items-center space-x-3">
                  {child.photoUrl && <img src={child.photoUrl} alt={child.name} className="w-8 h-8 rounded-full object-cover border-2 border-purple-200" />}
          ...
          <div className="bg-white/95 backdrop-blur-sm rounded-xl p-4 shadow-lg">
            ...
            <div className="grid grid-cols-1 gap-3 mb-4">
              <div>
                ...
                <Input type="email" value={formData.email} onChange={e => updateForm('email', e.target.value)} placeholder="votre@email.com" className="w-full px-3 py-2 rounded-lg border-2 border-slate-200 focus:border-purple-300 text-sm" />
              </div>
              <div>
                <label className="block text-slate-700 font-semibold mb-1 text-sm">Nom complet *</label>
                <Input value={formData.name} onChange={e => updateForm('name', e.target.value)} placeholder="Nom et prénom" className="w-full px-3 py-2 rounded-lg border-2 border-slate-200 focus:border-purple-300 text-sm" />
              </div>
                <div>
                  <label className="block text-slate-700 font-semibold mb-1 text-sm">Téléphone *</label>
                  <Input type="tel" value={formData.phone} onChange={e => updateForm('phone', e.target.value)} placeholder="+32 12 34 56 78" className="w-full px-3 py-2 rounded-lg border-2 border-slate-200 focus:border-purple-300 text-sm" />
                </div>
                {!isAuthenticated && <div>
                    ...
                    <div className="relative">
                      ...
                      {formData.password && <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-slate-400 hover:text-slate-600">
              ...
              <div>
                ...
                <Input value={formData.address} onChange={e => updateForm('address', e.target.value)} placeholder="Adresse complète" className="w-full px-3 py-2 rounded-lg border-2 border-slate-200 focus:border-purple-300 text-sm" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-700 font-semibold mb-1 text-sm">Ville *</label>
                  <Input value={formData.city} onChange={e => updateForm('city', e.target.value)} placeholder="Ville" className="w-full px-3 py-2 rounded-lg border-2 border-slate-200 focus:border-purple-300 text-sm" />
                </div>
                <div>
                  <label className="block text-slate-700 font-semibold mb-1 text-sm">Code postal *</label>
                  <Input value={formData.postalCode} onChange={e => updateForm('postalCode', e.target.value)} placeholder="75000" className="w-full px-3 py-2 rounded-lg border-2 border-slate-200 focus:border-purple-300 text-sm" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/95 backdrop-blur-sm rounded-xl p-4 shadow-lg">
              <Button onClick={handlePayment} disabled={!isFormValid || isProcessing} className={`
                  ...
                  ${isFormValid && !isProcessing ? 'bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 shadow-lg hover:shadow-xl transform hover:scale-105' : 'bg-gray-300 cursor-not-allowed'}
                `}>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-4 shadow-lg border border-green-100">
              <div className="flex items-center justify-center mb-4">
                <img src="https://i.ibb.co/9mjzbfTr/image.png" alt="Trustpilot" className="h-8 object-contain" />
              </div>
              <div className="text-center mb-4">
                <div className="flex items-center justify-center mb-2">
                  <div className="flex mr-2">
                    {[...Array(5)].map((_, i) => <Star key={i} className="w-5 h-5 text-green-500 fill-current" />)}
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                <div className="bg-green-50 p-3 rounded-lg border-l-4 border-green-400">
                  ...
                  <p className="text-slate-700 font-medium mb-1 text-sm">"Mon fils a reconnu son prénom dès la première page." Il m'a regardée avec de grands yeux et m'a dit : "C'est moi ?" Depuis, il veut qu'on relise le livre tous les soirs.</p>
                  <span className="text-slate-500 text-xs">⸻ Sophie M. ⭐️⭐️⭐️⭐️⭐️</span>
                </div>
                ...
                <div className="bg-green-50 p-3 rounded-lg border-l-4 border-green-400">
                  ...
                  <p className="text-slate-700 font-medium mb-1 text-sm">"Elle était super fière de se voir en héroïne." C'est la première fois qu'elle finit un livre d'un coup. Elle me l'a même raconté à sa manière à la crèche.</p>
                  <span className="text-slate-500 text-xs">⸻ Marie-Claire L. ⭐️⭐️⭐️⭐️⭐️</span>
                </div>
                ...
                <div className="bg-green-50 p-3 rounded-lg border-l-4 border-green-400">
                  ...
                  <p className="text-slate-700 font-medium mb-1 text-sm">"Il a eu un déclic." On a choisi le thème 'confiance'. Et franchement, je vois la différence. Il prend plus la parole et se tient plus droit.</p>
                  <span className="text-slate-500 text-xs">⸻ Camille R. ⭐️⭐️⭐️⭐️⭐️</span>
                </div>
              </div>
              ...
              <div className="border-t border-slate-200 pt-3">
                <div className="flex items-center justify-between text-sm text-slate-600">
                  ...
                  <a href="https://trustpilot.com" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline font-medium">
                    Voir tous les avis →
                  </a>
                </div>
              </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-6 shadow-lg sticky top-4">
              <div className="flex items-center mb-6">
                ...
                <h3 className="text-xl font-bold text-slate-700">Résumé de commande</h3>
              </div>
        ...
        })} variant="outline" className="w-full md:w-auto flex items-center gap-2 px-4 md:px-6 py-3 rounded-full bg-white/90 backdrop-blur-sm border-2 border-white/50 hover:bg-white/95 hover:border-purple-300 transition-colors text-sm md:text-base">
      ...
      <SignupModal isOpen={showSignupModal} onClose={() => setShowSignupModal(false)} onSignup={handleSignup} onSwitchToLogin={() => {
      setShowSignupModal(false);
