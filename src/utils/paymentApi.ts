...
export const initKonnectPayment = async (
  ...
  // Test bypass: If any amount is provided, go directly to success page for testing
  // This allows easy testing of the payment success page and PDF generation
  return {
    payUrl: `${window.location.origin}/payment-success?payment_ref=TEST-BYPASS-${Date.now()}`,
    paymentRef: `TEST-BYPASS-${Date.now()}`
  };

  // The code below is commented out for now to ensure the bypass always works for testing
  // Uncomment when ready to use real payment processing
  /*
  ...
  try {
    ...
    toast.error("Erreur de paiement", {
      description: error.message || "Une erreur s'est produite lors de l'initialisation du paiement"
    });
    throw error;
  }
  */
