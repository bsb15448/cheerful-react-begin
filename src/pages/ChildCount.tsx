...
const ChildCount = () => {
  ...
  const childOptions = [{
    count: 1,
    title: '1 héros',
    subtitle: '✨ Une aventure magique rien que pour lui',
    image: '/1.png'
  }, {
    count: 2,
    title: '2 héros',
    subtitle: '✨ Deux histoires uniques pour faire briller chacun',
    image: '/2.png'
  }, {
    count: 3,
    title: '3 héros',
    subtitle: '✨ Trois livres personnalisés, trois fois plus de fierté',
    image: '/3.png'
  }, {
    count: 4,
    title: '4 héros',
    subtitle: '✨ Quatre petits héros, quatre grandes aventures!',
    image: '/4.png'
  }];
        ...
        <div className="container mx-auto px-4 py-12 md:py-16 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            <h1 className="text-4xl md:text-5xl lg:text-7xl font-bold mb-6 leading-tight">
              <span className="text-slate-800">Combien de livres magiques voulez-vous créer ?</span>
              <br />
          ...
          <div className="max-w-3xl mx-auto space-y-4 md:space-y-6 mb-8 md:mb-12 px-4">
            {childOptions.map(option => <div key={option.count} onClick={() => handleCardClick(option.count)} className={`relative bg-cream-beige/90 backdrop-blur-sm rounded-2xl md:rounded-3xl p-0 cursor-pointer transition-all duration-300 transform hover:scale-[1.02] flex items-center shadow-xl border-2 overflow-hidden
                  ${selectedCount === option.count ? 'ring-4 ring-pastel-lavender shadow-2xl scale-[1.02] border-pastel-lavender bg-gradient-to-r from-pastel-lavender/20 to-powder-pink/20' : 'hover:shadow-2xl border-gray-100 hover:border-pastel-lavender hover:bg-gradient-to-r hover:from-pastel-lavender/10 hover:to-powder-pink/10'}
                `}>
                {selectedCount === option.count && <div className="absolute -top-3 -right-3 md:-top-4 md:-right-4 w-10 h-10 md:w-12 md:h-12 bg-pastel-lavender rounded-full flex items-center justify-center shadow-xl z-10">
                    <Check className="text-slate-700 w-5 h-5 md:w-6 md:h-6 font-bold" />
                  </div>}

                <div className="flex items-center justify-center w-full gap-4 md:gap-8 p-4 md:p-8">
                  <div className="flex-shrink-0">
                    <img src={option.image} alt="Héros de l'histoire" className="w-28 h-28 md:w-44 md:h-44 lg:w-52 lg:h-52 object-cover rounded-xl" />
                  </div>
                  
                  <div className="text-center">
                    <h3 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-2 md:mb-3 leading-tight text-slate-700">{option.title}</h3>
                    <p className="text-slate-500 text-sm md:text-lg lg:text-xl font-xs ml-[-7%] mt-[-2%]">{option.subtitle}</p>
                  </div>
                </div>
              </div>)}
          </div>
