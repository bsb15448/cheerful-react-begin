...
import { Car, Check, X, Wrench, Plus, Edit2, Fuel, Users, Calendar } from 'lucide-react';
...
const statusConfig: Record<string, { label: string; color: string; icon: typeof Check }> = {
  available: { label: 'Disponible', color: 'text-emerald-400 bg-emerald-500/10', icon: Check },
  in_use: { label: 'En service', color: 'text-blue-400 bg-blue-500/10', icon: Car },
  maintenance: { label: 'Maintenance', color: 'text-amber-400 bg-amber-500/10', icon: Wrench },
};
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl md:text-2xl font-semibold text-white">Véhicules</h1>
          <p className="text-sm text-white/40 mt-1">Gestion de votre flotte</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: 'En service', value: inUse, color: 'text-blue-400', bg: 'bg-blue-500/10' },
          { label: 'Maintenance', value: maint, color: 'text-amber-400', bg: 'bg-amber-500/10' },
        ].map((s) => (
            ...
            <div key={v.id} className="bg-[#14141f] border border-white/5 rounded-xl overflow-hidden hover:border-white/10 transition-colors">
              ...
              <div className="px-4 pb-4 space-y-3">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-white/5">
                  <span className="text-[10px] text-white/25">Prochain entretien: {v.nextService}</span>
