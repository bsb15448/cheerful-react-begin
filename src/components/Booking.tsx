      ...
      <div className="max-w-[1400px] mx-auto grid lg:grid-cols-[1fr_1.2fr] gap-16 lg:gap-24 relative">
        ...
        >
          ...
          <h2 className="font-display text-[clamp(2rem,4vw,3.2rem)] font-light leading-[1.1] mb-5">
            {t('booking.title1')}
            <br />
            <em className="text-gradient-gold italic font-semibold">{t('booking.title2')}</em>
          </h2>
          ...
          <div className="space-y-3 mb-10">
            {[t('booking.benefit1'), t('booking.benefit2'), t('booking.benefit3'), t('booking.benefit4')].map((item) => (
          ...
          <div className="card-glass inline-flex items-center gap-3 px-5 py-3.5 rounded-lg">
            ...
            <div>
              <span className="text-[12px] text-brand-cream/70 font-medium block">{t('booking.whatsappLabel')}</span>
        ...
        >
          {submitted ? (
            <div className="card-glass p-12 text-center rounded-lg">
              ...
              <div className="font-display text-2xl font-medium mb-3">{t('booking.successTitle')}</div>
              <p className="text-[14px] text-brand-cream/50">{t('booking.successDesc')}</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="card-glass p-8 lg:p-10 space-y-5 rounded-lg">
              <div className="grid sm:grid-cols-2 gap-5">
                <div>
                  <label className="text-[11px] tracking-[0.15em] uppercase text-brand-muted/60 mb-2 block font-medium">{t('booking.name')}</label>
                  <input name="name" type="text" placeholder={t('booking.namePlaceholder')} required value={formData.name} onChange={handleChange} className={inputClass} />
                </div>
                <div>
                  <label className="text-[11px] tracking-[0.15em] uppercase text-brand-muted/60 mb-2 block font-medium">{t('booking.phone')}</label>
              ...
              <div>
                <label className="text-[11px] tracking-[0.15em] uppercase text-brand-muted/60 mb-2 block font-medium">{t('booking.email')}</label>
              ...
              <div className="grid sm:grid-cols-2 gap-5">
                <div>
                  <label className="text-[11px] tracking-[0.15em] uppercase text-brand-muted/60 mb-2 block font-medium">{t('booking.date')}</label>
                  <input name="date" type="date" required value={formData.date} onChange={handleChange} className={inputClass} />
                </div>
                <div>
                  <label className="text-[11px] tracking-[0.15em] uppercase text-brand-muted/60 mb-2 block font-medium">{t('booking.tripType')}</label>
                  <select name="service" required value={formData.service} onChange={handleChange} className={`${inputClass} bg-transparent`} style={{ backgroundColor: 'transparent' }}>
                    <option value="" className="bg-brand-charcoal text-brand-cream">{t('booking.tripSelect')}</option>
                    <option value="airport" className="bg-brand-charcoal text-brand-cream">{t('booking.tripAirport')}</option>
                    <option value="business" className="bg-brand-charcoal text-brand-cream">{t('booking.tripBusiness')}</option>
                    <option value="event" className="bg-brand-charcoal text-brand-cream">{t('booking.tripEvent')}</option>
                    <option value="excursion" className="bg-brand-charcoal text-brand-cream">{t('booking.tripExcursion')}</option>
                    <option value="other" className="bg-brand-charcoal text-brand-cream">{t('booking.tripOther')}</option>
                  </select>
                </div>
              </div>

              <div>
                <label className="text-[11px] tracking-[0.15em] uppercase text-brand-muted/60 mb-2 block font-medium">{t('booking.message')}</label>
                <textarea
                  name="message"
