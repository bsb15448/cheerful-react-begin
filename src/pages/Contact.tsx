...
const Contact = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-b from-orange-50 to-pink-50" style={{
    background: 'linear-gradient(135deg, #E8D5FF 0%, #F3E8FF 25%, #E0E7FF 50%, #F0F4FF 75%, #F8FAFF 100%)'
  }}>"
    <div className="mt-[-6%] md:mt-[-1.8%]">
  <Header />
</div>
      ...
      <main className="pt-28 md:pt-40 pb-16 px-4">
        <div className="max-w-2xl mx-auto">
          ...
          <div style={{
          background: 'linear-gradient(135deg, #E8D5FF 0%, #F3E8FF 25%, #E0E7FF 50%, #F0F4FF 75%, #F8FAFF 100%)'
        }} className="rounded-2xl p-8 md:p-12 shadow-2xl border border-purple-200 mt-[-25%]">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Input name="name" type="text" placeholder="Nom complet *" value={formData.name} onChange={handleInputChange} required disabled={isSubmitting} className="h-14 border-2 border-purple-200/60 focus:border-purple-500 focus:ring-purple-400/20 bg-white/95 hover:bg-white transition-all duration-200 rounded-xl text-slate-800 placeholder:text-slate-500 font-medium text-base shadow-sm hover:shadow-md focus:shadow-lg" />
                </div>
                
                <div>
                  <Input name="email" type="email" placeholder="Adresse e-mail *" value={formData.email} onChange={handleInputChange} required disabled={isSubmitting} className="h-14 border-2 border-purple-200/60 focus:border-purple-500 focus:ring-purple-400/20 bg-white/95 hover:bg-white transition-all duration-200 rounded-xl text-slate-800 placeholder:text-slate-500 font-medium text-base shadow-sm hover:shadow-md focus:shadow-lg" />
                </div>
              </div>

              {/* Second row - Phone */}
              <div>
                <Input name="phone" type="tel" placeholder="Numéro de téléphone (optionnel)" value={formData.phone} onChange={handleInputChange} disabled={isSubmitting} className="h-14 border-2 border-purple-200/60 focus:border-purple-500 focus:ring-purple-400/20 bg-white/95 hover:bg-white transition-all duration-200 rounded-xl text-slate-800 placeholder:text-slate-500 font-medium text-base shadow-sm hover:shadow-md focus:shadow-lg" />
              </div>

              {/* Third row - Message */}
              <div>
                <Textarea name="message" placeholder="Votre message *" value={formData.message} onChange={handleInputChange} required disabled={isSubmitting} rows={5} className="border-2 border-purple-200/60 focus:border-purple-500 focus:ring-purple-400/20 bg-white/95 hover:bg-white transition-all duration-200 rounded-xl text-slate-800 placeholder:text-slate-500 font-medium text-base shadow-sm hover:shadow-md focus:shadow-lg resize-none min-h-[120px]" />
              </div>
          ...
          <div className="mt-12 rounded-2xl p-8 md:p-12 shadow-2xl border border-purple-200" style={{
